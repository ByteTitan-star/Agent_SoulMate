...
def _resolve_time_window(request):
    ...
    if range_key in {'today', 'day'}:
        ...
        start_day = (local_end - timedelta(days=days - 1)).replace(hour=0, minute=0, second=0, microsecond=0)
        start_dt = start_day
        ...
        local_now = timezone.localtime(now)
        end_dt = now
        start_dt = (local_now - timedelta(days=29)).replace(hour=0, minute=0, second=0, microsecond=0)
