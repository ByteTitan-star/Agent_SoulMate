"""
WebSocket chat consumer with full-duplex barge-in.

Client protocol (JSON text frames):
  - {type: "message", content: "..."}
  - {type: "vad_start"} / {type: "speech_start"}  -> cancel LLM/TTS immediately
  - {type: "audio_chunk", data: "<base64>"}       -> append audio chunk
  - {type: "vad_end"}                             -> ASR + reply
  - {type: "interrupt"}                           -> cancel current tasks

Binary frames are treated as a complete audio utterance (legacy).
"""
...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        await self.accept()
        await self.send_json({"type": "ready", "character_id": self.character_id})
    ...
    async def _run_reply(self, user_text: str, source: str):
        ...
        try:
            ...
            if full_text.strip() and getattr(settings, "ELEVENLABS_API_KEY", ""):
    ...
    async def _interrupt_current_generation(self, reason: str = "interrupt", notify: bool = True):
        async with self._task_lock:
            ...
            for task in (self.reply_task, self.tts_task):
                if not task:
                    continue
                try:
                    await task
