...
# Skills live in backend/skills as a package. Import failures degrade gracefully.
try:
...
def build_system_prompt(character) -> str:
    ...
    if getattr(character, 'opening_message', None):
        parts.append(
            '开场白仅用于会话开始时的欢迎，用户已经提问后禁止再复述或改写开场白；'
            f'开场白原文：{character.opening_message}'
        )
    if isinstance(getattr(character, 'personality', None), list) and character.personality:
        parts.append('性格标签：' + '、'.join(character.personality[:10]))
    parts.append(
...
def _get_rag_context(rag_retriever, user_message: str) -> str:
    ...
    try:
        ...
        docs = (
            rag_retriever.invoke(user_message)
            if hasattr(rag_retriever, 'invoke')
            else rag_retriever.get_relevant_documents(user_message)
        )
        if not docs:
            return ''
        chunks = []
    ...
    try:
        with open(TOKEN_MD_PATH, 'w', encoding='utf-8') as f:
            ...
            for log in reversed(logs):
                f.write(
                    f"| {log['time']} | {log['project']} | {log['model']} | {log['prompt']} | {log['completion']} | {log['total']} |\n"
                )
    except Exception as e:
