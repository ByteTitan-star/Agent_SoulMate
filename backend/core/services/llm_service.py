...
# Token usage log path. Configurable via TOKEN_LOG_PATH so the service is
# portable (no machine-specific absolute paths in source). Defaults to a
# project-local file under backend/data; set TOKEN_LOG_PATH in .env to keep a
# shared cross-project log (e.g. D:\...\Token记录\token.md).
TOKEN_MD_PATH = os.environ.get(
    "TOKEN_LOG_PATH",
    os.path.join(settings.BASE_DIR, "data", "token_usage.md"),
)


# ================= Runtime skills (weather / news) =================
# Skills now live inside the backend as a proper package (backend/skills/), so
# we import them as modules instead of mutating sys.path. Import failures
# degrade gracefully: chat keeps working, only tool calls report "not loaded".
try:
    from skills.news_skill import get_realtime_news
    from skills.weather_skill import get_realtime_weather

    SKILLS_LOADED = True
except ImportError as exc:  # pragma: no cover - depends on optional deps/httpx
    print(f"警告：无法加载 Skill 模块：{exc}")
    SKILLS_LOADED = False
...
def _get_rag_context(rag_retriever, user_message: str) -> str:
    ...
    try:
        ...
        if hasattr(rag_retriever, 'invoke'):
            docs = rag_retriever.invoke(user_message)
        else:
            docs = rag_retriever.get_relevant_documents(user_message)
        if not docs:
            return ''
        chunks = []
